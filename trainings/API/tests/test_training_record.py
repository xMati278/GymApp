...
class TestTrainingRecordApi(TestCase):
    ...
    def test_CreateTrainingRecord_returns_201_when_provided_correct_data(self):
        ...
        response = self.client.post(reverse('api-training-records'), data=data, content_type='application/json',
    ...
    def test_ReadTrainingsRecord_returns_200_when_provided_correct_data(self):
        response = self.client.get(reverse('api-training-records'), headers=self.auth_header)
        response_data = json.loads(response.content)
    ...
    def test_ReadTrainingsRecord_returns_401_when_not_provided_authorization_token(self):
        response = self.client.get(reverse('api-training-records'))

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_UpdateTrainingRecord_returns_201_when_provided_correct_data(self):
        data = {'value': 150}
        response = self.client.patch(reverse('api-edit-training-record', kwargs={'pk': self.user_record.pk}), data=data,
    ...
    def test_DeleteTrainingRecord_returns_401_when_authorization_token_not_provided(self):
        response = self.client.delete(reverse('api-edit-training-record', kwargs={'pk': self.user_record.pk}),
                                      content_type='application/json')
