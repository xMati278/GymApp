...
class TestUserTrainingPlanApi(TestCase):
    ...
    def test_CreateUserTrainingPlan_returns_201_when_provided_correct_data(self):
        data = {'name': 'Extra GPP'}
        response = self.client.post(reverse('api-user-training-plans-list'), data=data, content_type='application/json',
    ...
    def test_ReadUserTrainingPlans_returns_200_when_provided_correct_data(self):
        response = self.client.get(reverse('api-user-training-plans-list'), headers=self.auth_header)
        response_data = json.loads(response.content)
    ...
    def test_ReadUserTrainingPlans_returns_401_when_not_provided_authorization_token(self):
        response = self.client.get(reverse('api-user-training-plans-list'))

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    def test_UpdateUserTrainingPlans_returns_200_when_provided_correct_data(self):
        data = {'name': "Push Test"}
        response = self.client.patch(reverse('api-user-training-plans-detail', kwargs={'pk': self.trainingplan.pk}), data=data,
    ...
    def test_UpdateUserTrainingPlans_returns_401_when_authorization_token_not_provided(self):
        data = {'name': "Push Test"}
        response = self.client.patch(reverse('api-user-training-plans-detail', kwargs={'pk': 1}), data=data,
                                     content_type='application/json')

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_DeleteUserTrainingPlan_returns_401_when_authorization_token_not_provided(self):
        response = self.client.delete(reverse('api-user-training-plans-detail', kwargs={'pk': 1}),
                                      content_type='application/json')

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_DeleteUserTrainingPlan_returns_204_when_authorization_token_provided(self):
        response = self.client.delete(reverse('api-user-training-plans-detail', kwargs={'pk': self.trainingplan.pk}),
