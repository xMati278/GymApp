...
class TestTraining(TestCase):
    ...
    def test_CreateTraining_returns_201_when_provided_correct_data(self):
        ...
        response = self.client.post(reverse('api-trainings'), data=data, content_type='application/json',
    ...
    def test_ReadTrainings_returns_200_when_provided_correct_data(self):
        response = self.client.get(reverse('api-trainings'), headers=self.auth_header)
        response_data = json.loads(response.content)
    ...
    def test_ReadTrainings_returns_401_when_not_provided_authorization_token(self):
        response = self.client.get(reverse('api-trainings'))

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_UpdateTraining_returns_201_when_provided_correct_data(self):
        data = {'note': 'ads'}
        response = self.client.patch(reverse('api-trainings-edit', kwargs={'pk': self.training.pk}), data=data,
    ...
    def test_UpdateTraining_returns_401_when_authorization_token_not_provided(self):
        data = {'user': "2"}
        response = self.client.patch(reverse('api-trainings-edit', kwargs={'pk': 1}), data=data,
                                     content_type='application/json')

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_DeleteTraining_returns_401_when_authorization_token_not_provided(self):
        response = self.client.delete(reverse('api-trainings-edit', kwargs={'pk': 1}),
                                      content_type='application/json')

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_DeleteTraining_returns_204_when_authorization_token_provided(self):
        response = self.client.delete(reverse('api-trainings-edit', kwargs={'pk': self.training.pk}),
