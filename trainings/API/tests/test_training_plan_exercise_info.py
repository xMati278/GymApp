...
class TestTrainingPlanExerciseInfoApi(TestCase):
    ...
    def test_CreateTrainingPlanExerciseInfo_returns_201_when_provided_correct_data(self):
        ...
        response = self.client.post(reverse('api-training-plan-exercise-info-list'), data=data, content_type='application/json',
    ...
    def test_ReadTrainingPlanExerciseInfo_returns_200_when_provided_correct_data(self):
        response = self.client.get(reverse('api-training-plan-exercise-info-list'), headers=self.auth_header)
        response_data = json.loads(response.content)
    ...
    def test_ReadTrainingPlanExerciseInfo_returns_401_when_not_provided_authorization_token(self):
        response = self.client.get(reverse('api-training-plan-exercise-info-list'))

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_UpdateTrainingPlanExerciseInfo_returns_201_when_provided_correct_data(self):
        data = {'reps': 5}
        response = self.client.patch(reverse('api-training-plan-exercise-info-detail', kwargs={'pk': self.exercise_info.pk}), data=data,
    ...
    def test_DestroyTrainingPlanExerciseInfo_returns_401_when_authorization_token_not_provided(self):
        response = self.client.delete(reverse('api-training-plan-exercise-info-detail', kwargs={'pk': self.exercise_info.pk}),
                                      content_type='application/json')
